# https://raw.githubusercontent.com/tuupola/micropython-examples/master/mpu9250-fusion/firmware/lib/fusion.py
...
class Fusion(object):
    ...
    def calibrate(self, getmagxyz, count = 256, wait=0):
        magmax = list(getmagxyz())             # Initialise max and min lists with current values
        magmin = magmax[:]
        while count:
            if wait != 0:
                if callable(wait):
